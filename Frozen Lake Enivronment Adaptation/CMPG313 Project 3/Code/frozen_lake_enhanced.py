    ...
    * v0: Initial version release
    ...
    def _render_gui(self, mode):
        ...
        if self.hole_img is None:
            file_name = path.join(path.dirname(__file__), "pothole.png")
            self.hole_img = pygame.transform.scale(
                pygame.image.load(file_name), self.cell_size
            )
        if self.cracked_hole_img is None:
            file_name = path.join(path.dirname(__file__), "car_crash.png")
            self.cracked_hole_img = pygame.transform.scale(
                pygame.image.load(file_name), self.cell_size
            )
        if self.ice_img is None:
            file_name = path.join(path.dirname(__file__), "road.png")
            self.ice_img = pygame.transform.scale(
                pygame.image.load(file_name), self.cell_size
            )
        if self.goal_img is None:
            file_name = path.join(path.dirname(__file__), "goal_state.png")
            self.goal_img = pygame.transform.scale(
                pygame.image.load(file_name), self.cell_size
            )
        if self.start_img is None:
            file_name = path.join(path.dirname(__file__), "home.png")
            self.start_img = pygame.transform.scale(
                pygame.image.load(file_name), self.cell_size
            )
        if self.elf_images is None:
            elfs = [
                path.join(path.dirname(__file__), "car_left.png"),
                path.join(path.dirname(__file__), "car_down.png"),
                path.join(path.dirname(__file__), "car_right.png"),
                path.join(path.dirname(__file__), "car_left.png"),
            ]
            self.elf_images = [
